...
class IBTestCase(testcases.common.TestCase):
    def __loadBarFeed(self, timeZone=None):
        ...
        ret.addBarsFromCSV("bac", testcases.common.get_data_file_path("ib-bac_2000-p20160819.csv"))
        ret.loadAll()
        return ret

    def testBaseFeedInterface(self):
        barFeed = ibfeed.Feed()
        barFeed.addBarsFromCSV("bac", testcases.common.get_data_file_path("ib-bac_2000-p20160819.csv"))
    ...
    def testWithIntegerTimezone(self):
        ...
        try:
            barFeed = ibfeed.Feed()
            barFeed.addBarsFromCSV("bac", testcases.common.get_data_file_path("ib-bac_2000-p20160819.csv"), -5)
            self.assertTrue(False, "Exception expected")
        except Exception, e:
            self.assertTrue(str(e).find("timezone as an int parameter is not supported anymore") == 0)
    ...
    def testReset(self):
        instrument = "bac"
        barFeed = ibfeed.Feed()
        barFeed.addBarsFromCSV(instrument, testcases.common.get_data_file_path("ib-bac_2000-p20160819.csv"))
